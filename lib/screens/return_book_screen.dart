...
class _ReturnBookScreenState extends State<ReturnBookScreen> {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('借阅人: ${record.borrowerName} (${record.borrowerType})'),
  ...
  List<BorrowRecord> get _filteredRecords {
    ...
    return _borrowRecords.where((record) {
      return (record.bookTitle?.toLowerCase() ?? '').contains(
            _searchQuery.toLowerCase(),
          ) ||
          (record.studentName?.toLowerCase() ?? '').contains(
            _searchQuery.toLowerCase(),
          ) ||
          (record.borrowerTeacherName?.toLowerCase() ?? '').contains(
            _searchQuery.toLowerCase(),
          ) ||
          (record.teacherName?.toLowerCase() ?? '').contains(
            _searchQuery.toLowerCase(),
          );
    }).toList();
  }
                ...
                : _filteredRecords.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final daysOverdue =
                              isOverdue && record.dueDate != null
                                  ? DateTime.now()
                                      .difference(record.dueDate!)
                                      .inDays
                                  : 0;
